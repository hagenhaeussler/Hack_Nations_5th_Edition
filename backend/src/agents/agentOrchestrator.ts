...
export function sourceToPaper(source: ResearchSource, index: number): Paper {
  return {
    ...
    pdfUrl: typeof source.metadata.pdf_url === "string" ? source.metadata.pdf_url : undefined,
...
export function paperToResearchSource(paper: Paper): ResearchSource {
  return {
    ...
    metadata: {
      venue: paper.venue,
      provider: paper.provider ?? "project_papers",
      pdf_url: paper.pdfUrl ?? null,
      referenced_openalex_ids: paper.referencedPaperIds ?? [],
      related_openalex_ids: paper.relatedPaperIds ?? [],
    },
...
export function experimentPlanToFinalPlan(args: {
  ...
  const finalPlan: FinalExperimentPlan = {
    ...
    agent_notes: [
      ...
      ...(args.benchmarkInsightCount && args.benchmarkInsightCount > 0
        ? [
            `Benchmark-informed improvements applied from ${args.benchmarkInsightCount} prior researcher evaluation insight${args.benchmarkInsightCount === 1 ? "" : "s"}.`,
          ]
        : []),
      ...parsed.setup_warnings,
    ],
...
export async function runCreatorPlanAgents(args: {
  ...
  const benchmarkInsights = await getBenchmarkRepo().getRelevantInsights({
    domain: extraction.domain,
    experiment_type: extraction.experiment_type,
    project_id: args.project.id,
    query: args.project.hypothesis,
    limit: 10,
  });
  const fallbackPlan = () =>
  ...
  let planRun = await generatePlanWithOpenAI({
    ...
    benchmarkInsights: {
      heading: "Relevant Benchmark Feedback From Previous Researcher Evaluations",
      insights: benchmarkInsights.map((insight) => ({
        insight_text: insight.insight_text,
        category_tags: insight.category_tags,
        applies_to: insight.applies_to,
        recommended_creator_agent_adjustments:
          insight.structured_insight.recommended_creator_agent_adjustments,
  ...
  try {
    finalPlan = experimentPlanToFinalPlan({
      ...
      lessons: args.lessons,
      generationMode,
      benchmarkInsightCount: benchmarkInsights.length,
    });
  } catch (err) {
    warnings.push(
