...
export interface BenchmarkInsight {
  ...
  experiment_type: string | null;
  insight_text: string;
  structured_insight: Record<string, unknown>;
...
export interface SubmitBenchmarkEvaluationRequest {
  project_id: string;
  plan_id: string;
  scores: BenchmarkScores;
  written_feedback?: string | null;
...
export interface SubmitBenchmarkEvaluationResponse {
  success: true;
...
export async function submitBenchmarkEvaluation(
  planId: string,
  request: SubmitBenchmarkEvaluationRequest,
): Promise<SubmitBenchmarkEvaluationResponse> {
  return jsonRequest<SubmitBenchmarkEvaluationResponse>(
    `/api/plans/${encodeURIComponent(planId)}/evaluations`,
    { method: "POST", body: request },
  );
}

export async function listBenchmarkEvaluations(): Promise<BenchmarkEvaluation[]> {
  const res = await jsonRequest<{ evaluations: BenchmarkEvaluation[] }>(
    "/api/benchmark/evaluations",
  );
...
export async function getBenchmarkEvaluation(
  ...
  const res = await jsonRequest<{ evaluation: BenchmarkEvaluation }>(
    `/api/benchmark/evaluations/${encodeURIComponent(evaluationId)}`,
  );
  return res.evaluation;
}

export async function getBenchmarkSummary(): Promise<BenchmarkSummary> {
  const res = await jsonRequest<BenchmarkSummary>("/api/benchmark/summary");
  return res;
}
