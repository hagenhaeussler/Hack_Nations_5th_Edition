...
export function RiskAnalyzerModal({
  ...
  return (
    ...
    >
      <section className="flex max-h-[88vh] w-full max-w-[980px] flex-col overflow-hidden rounded-xl border border-[color:var(--border-default)] bg-bg-surface shadow-lg">
        <header className="flex items-start gap-3 border-b border-[color:var(--border-default)] px-5 py-4">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-sm bg-[color:var(--accent-subtle)] text-accent">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h2 className="mt-0.5 text-[20px] font-medium tracking-[-0.01em] text-text-primary">
              Risk Analyzer
            </h2>
            <p className="mt-1 text-[12.5px] leading-[1.5] text-text-secondary">
              {analysis
                ? `Overall risk: ${capitalise(analysis.overall_risk_level)}. ${analysis.summary}`
          ...
          <button
            ...
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-sm text-text-tertiary transition-colors hover:bg-bg-hover hover:text-text-primary"
          >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {loading ? (
            <div className="flex min-h-64 flex-col items-center justify-center text-center">
              <Loader2 size={24} strokeWidth={1.75} className="animate-spin text-accent" />
              <p className="mt-3 text-[13px] font-medium text-text-primary">
                Analyzing project risks...
              </p>
              <p className="mt-1 max-w-[42ch] text-[12px] leading-[1.5] text-text-secondary">
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        {(["critical", "high", "medium", "low"] as const).map((severity) => (
          ...
          >
            <p className="font-medium">{capitalise(severity)}</p>
            <p className="mt-0.5 text-[18px] font-light leading-none">
      ...
      {analysis.top_risks.length === 0 ? (
        <p className="rounded-md border border-[color:var(--border-default)] bg-bg-primary p-4 text-[13px] text-text-secondary">
      ...
      {analysis.recommended_next_actions.length > 0 ? (
        <footer className="rounded-md border border-[color:var(--border-default)] bg-bg-primary p-4">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-text-tertiary">
            Recommended next actions
          </p>
          <ul className="mt-2 flex flex-col gap-1.5">
            {analysis.recommended_next_actions.map((action) => (
              <li key={action} className="text-[12.5px] leading-[1.5] text-text-secondary">
                {action}
              </li>
            ))}
          </ul>
...
function RiskCard({
  ...
  return (
    <li className="rounded-lg border border-[color:var(--border-default)] bg-bg-primary p-4">
      <article>
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h3 className="mt-1 text-[15px] font-medium leading-[1.35] text-text-primary">
              {risk.title}
            </h3>
          </div>
        ...
        <div className="mt-3 grid gap-3 text-[12.5px] leading-[1.5] text-text-secondary md:grid-cols-[1.2fr_0.8fr]">
          <div>
          ...
          <div className="rounded-md border border-[color:var(--border-default)] bg-bg-surface p-3">
            <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-text-tertiary">
              Mitigation
            </p>
            <ul className="mt-2 flex flex-col gap-1.5">
        ...
        <details className="mt-3 text-[12px] text-text-secondary">
          <summary className="cursor-pointer text-text-tertiary">Evidence and consequences</summary>
        ...
        {firstNode && onHighlightStep ? (
          <button
            ...
            className="mt-3 rounded-sm border border-[color:var(--border-default)] bg-bg-surface px-3 py-1.5 text-[12px] font-medium text-text-secondary transition-colors hover:border-[color:var(--accent)] hover:bg-[color:var(--accent-subtle)] hover:text-accent"
          >
            Highlight step
          </button>
        ) : null}
